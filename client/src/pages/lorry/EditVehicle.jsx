...
const EditVehicle = () => {
  ...
  const fetchVehicle = async () => {
    try {
      const response = await lorryAPI.getLorryById(id)
      const vehicle = response.data.lorry
      
      setFormData({
        vehicleNumber: vehicle.vehicleNumber || '',
        vehicleType: vehicle.vehicleType || '',
        capacity: vehicle.capacity || '',
        currentLocationCity: vehicle.currentLocation?.city || '',
        currentLocationState: vehicle.currentLocation?.state || '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      submitData.append('driverPhone', formData.driverPhone)
      ...
      const response = await lorryAPI.updateLorry(id, submitData)
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {/* Document Upload */}
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          <h2 className="text-xl font-semibold text-gray-900 mb-4">Update Documents (Optional)</h2>
          <p className="text-sm text-gray-600 mb-4">Upload new documents only if you want to replace existing ones</p>
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          <div className="flex">
            <svg className="w-5 h-5 text-blue-700 mr-3 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            <div>
              <h3 className="text-sm font-medium text-blue-800 mb-1">Update Notice</h3>
              <p className="text-sm text-blue-700">
                Changes to vehicle details will be reviewed by the admin team if verification is required.
              </p>
            </div>
          </div>
        </div>
